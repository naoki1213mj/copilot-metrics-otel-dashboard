...
| `chat_panel_edit_mode` | Plan モードのインタラクション数（API フィールド名は旧称 edit のまま） |
| `chat_panel_custom_mode` | Custom Agent のインタラクション数 |
| `agent_edit` | Agent/Plan モードでの行追加・削除 |
| `monthly_active_agent_users` | IDE Agent モードの月次アクティブユーザー数 |
| `copilot_coding_agent_active_users_1d` | Copilot cloud agent の当日アクティブユーザー数（nullable、2026-04 追加） |
| `copilot_coding_agent_active_users_7d` | Copilot cloud agent の直近 7 日アクティブユーザー数（nullable、2026-04 追加） |
| `copilot_coding_agent_active_users_28d` | Copilot cloud agent の直近 28 日アクティブユーザー数（nullable、2026-04 追加） |
...
| `used_copilot_coding_agent` | Copilot cloud agent を利用したか |
| `used_copilot_code_review_active` | code review に能動的に関与したか（2026-04 追加） |
| `used_copilot_code_review_passive` | code review が自動実行されたか（2026-04 追加） |

## 最近の API 拡張（2026 年 4 月）

GitHub は Copilot usage metrics API を継続的に拡張中。記事執筆時点で押さえておくべき動き:

- 2026-04-10: Copilot cloud agent のアクティブユーザー集計フィールド追加
- 2026-04-10: Copilot CLI データが totals / feature breakdowns に統合
- 2026-04-08: Copilot レビュー済み PR のマージメトリクス追加
- 2026-04-06: code review ユーザーの active / passive 区別
- 2026-04-02: Organization レポートに per-user CLI activity 追加
- 2026-03-05: user-level CLI activity 追加
- 2026-02-27: enterprise-level CLI activity 追加（GA タイミング）

実装時は最新のフィールド一覧を https://docs.github.com/en/rest/copilot/copilot-usage-metrics で確認すること。
