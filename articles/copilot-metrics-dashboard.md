...
正直なところ、API のレスポンスが NDJSON（後述します）という少し変わった形式だったので、最初に Copilot CLI が生成したコードでは `response.json()` で全体をパースしようとして失敗しました。ただ、エラーメッセージを渡したら「NDJSON は行ごとにパースする必要がある」と修正してくれたので、ここは CLI のエラー修正能力に助けられた場面です。
...
### NDJSON を JSON 配列と勘違いした

最初に Copilot CLI が生成したコードでは `response.json()` で全体をパースしようとして `JSONDecodeError` が出ました。新 API のダウンロードリンク先は NDJSON 形式なので、JSON 配列とは別物です。

polars の `pl.read_ndjson()` を知ってからはスムーズでしたが、知らなかったら `json.loads()` のループを自分で書くところでした。GitHub Community の Discussion でもここで困っている人が多かったです。

### Organization レポートのフィールドが欠けることがある

公式ドキュメントにはフィールドの一覧が載っていますが、Organization の規模や Copilot の利用状況によっては、一部のフィールドが含まれないレコードがあるようです。polars の場合、存在しないカラムにアクセスすると `ColumnNotFoundError` になるので、`with_columns(pl.lit(0).alias(col))` で事前に 0 埋めしておくのが安全です。
...
個人的に面白いと感じたのは、Agent モードの利用状況が見えるようになったことです。`chat_panel_agent_mode` フィールドで、Organization 内で Agent モードがどのくらい使われているか分かります。Ask モードとの比率の推移を追うと、組織内の AI 活用の進み具合が見えてきます。

ユーザー単位のレポートも取れるようになったので、「ライセンスは付与されているけど実際には使っていない人」を特定するのもやりやすくなりました。テックリードがライセンスの最適化を考えるときに、ここがいちばん知りたいところではないかと思います。
...
OTel を入れたことで、`uv run src/fetch_metrics.py` を実行すると、データ取得プロセスのトレースが Application Insights に送信されます。Azure Portal で Application Insights を開き、「トランザクション検索」を見ると、こんな情報が確認できます。
