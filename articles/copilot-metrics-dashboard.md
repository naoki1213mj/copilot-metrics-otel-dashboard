...
GitHub Copilot の利用状況を Organization 単位で眺めるダッシュボードを作ってみました。新しい Copilot usage metrics API でデータを取得し、Python と polars で加工して、React で描画する構成です。Azure Functions で取り込みを回し、Azure App Service でダッシュボードを配信するところまで組んでいます。

ただし、今の環境では会社のポリシーで実 Organization のデータを取れません。この記事で見せているのはすべてモックデータです。API のフィールド定義に沿ったモックを用意して、「実データが入ったらそのまま差し替わる」構成にしてあるので、あくまで「やるならこう組む」の検証として読んでもらえると助かります。実 Organization データを使った Azure 上の検証は未検証です。
...
ここからは実装してみての感想です。28 日 latest は「全体像をさっと見たい」用で、日次のほうは「好きな粒度で束ねてくれ」という割り切りなのだろう、と感じました。
...
Azure Key Vault → GitHub token 保持
Azure Log Analytics → 全リソースの diagnostic settings
```
...
Bicep は subscription スコープで書いていて、リソースグループの作成から始めて、Azure Log Analytics、Storage、Cosmos DB、Azure Key Vault、App Service Plan、Web App、Function App、RBAC まで一通りモジュール化しました。Storage と Cosmos DB はキー認証を無効にしていて、Function App と Web App の Managed Identity に必要な RBAC ロールを付けています。Azure Functions 側には Blob Data Contributor と Cosmos DB Data Contributor に加えて、host storage 用の Storage Blob Data Owner / Storage Table Data Contributor、Azure Key Vault 用の Secrets Officer も付けています。Web App 側は dashboard-data コンテナーの Blob Data Reader です。

GitHub token は Azure Key Vault に入れて、`azd env set --file .env` で流し込む形です。

デプロイの流れはこう。

```bash:ターミナル
azd env new <your-env-name> --subscription <id> --location eastasia
azd env set --file .env
azd provision --no-state
azd deploy --all
```

正直に書くと、今の subscription では App Service plan の quota 制限に当たって、region や SKU を何度か変えました。最終的に East Asia で provision は通り、App Service と Function App の両方が動くところまで持っていけました。

途中でいちばん手こずったのは Azure Functions 側です。最初は `0 functions found` が続いて、host storage や Oryx build を疑っていました。実際には、Python v2 model の route handler で `req` ではなく `_req` を引数名にしていたのが原因でした。ローカルでは見えにくかったのですが、Azure 側 worker の indexing で `FunctionLoadError` になっていて、そこを直したら `GET /api/ingestion/status` と `GET /api/data/daily_summary.json` が返るようになった。

このへんは「Azure に載せてみて初めてわかる癖」でした。手元の import が通ることと、Azure Functions の worker indexing が通ることは別物だと痛感しました。

## 監視

Azure Application Insights と Azure Log Analytics はかなり厚めに入れました。Function App と Web App だけでなく、Storage や Cosmos DB にも diagnostic settings を付けています。

`function_app.py` には ingestion 用の endpoint も用意しました。ここは認証要件が少し違います。

- `POST api/ingestion/run` — 手動で取り込みを実行。function key が必要
- `GET api/ingestion/status` — 最新の取り込み状態を返す。function key が必要
- `GET api/data/{file_name}` — ダッシュボード用 JSON を配信。ここだけ匿名

この記事を書いている時点では、Azure 上で次までは確認できています。ここでの確認はすべてモックデータ前提で、実 Organization データでの Azure 検証は未検証です。

- App Service のダッシュボードが 200 を返す
- Function App の `GET /api/ingestion/status` が function key 付きで `ready` を返す
- `POST /api/ingestion/run` が function key 付きで mock ingestion に成功する
- `GET /api/data/daily_summary.json` が 200 で返る

つまり、「実データは使えないが、mock データなら Azure 実機で end-to-end に回る」と言える状態です。検証記事としてはここまで確認できていれば十分かなと考えています。
...
モックベースの検証ですが、Azure 実機で ingestion と配信が通るところまでは確認できました。会社のポリシー上、実 Organization データの検証はまだできません。ただ、「usage metrics API のデータをどう取り込み、どう可視化し、Azure にどう載せるか」という流れはかなり具体的に残せたと思っています。

次にやりたいのは、実データが使える環境でそのまま差し替えて差分を確認することと、Cosmos DB に蓄積した run 履歴から adoption の変化を追えるようにすることです。
