...
- Agent モード / Ask モード / Plan モードの利用比率
- ユーザーごとの利用状況
...
`agent_edit` フィールド（Agent/Plan モードでの行追加・削除）も追加されていて、「AI がどれだけコードを実際に書いたか」の定量化ができそうです。ここは自分ではまだ活用できていませんが、Copilot の ROI 議論ではキーになるフィールドだと思います。
